...
def main() -> None:
    ...
    parser.add_argument("--bundle-output", default="outputs/training_bundle/seam_residual_corrector_training_bundle.tar.gz")
    ...
    materialize_cmd = [py, "-m", "scripts.materialize_strip_dataset", "--manifest", str(root / "manifests/input_raw_manifest.jsonl"), "--out", str(root / "outputs/strip_cache"), "--splits", "train", "val"]
    if workers > 0:
        prep_cmd += ["--workers", str(workers)]
        materialize_cmd += ["--workers", str(workers)]
    ...
    subprocess.run(materialize_cmd, check=True)
