...
def build_notebook() -> dict:
    cells = [
        ...
        code(
            ...
            "RUNTIME_ZIP_URL = ''\n"
            "USE_RAMDISK = True\n"
            "# How many GB to reserve for the ramdisk. The code always leaves ≥64 GB free\n"
            "# for training, so setting this larger than (available_RAM - 64 GB) has no effect.\n"
            "# Dataset is ~2 GB extracted; 8 GB is plenty. Increase only if you pre-cache more data.\n"
            "RAMDISK_SIZE_GB = 8\n"
            ...
            "TRAIN_CONFIG_NAME = 'finetune_harmonizer_v1.yaml'\n"
            "# ── Checkpoint strategy (pick ONE, set the other to '') ─────────────────\n"
            "# LOAD_WEIGHTS: start a NEW run with pretrained weights (fresh optimizer/LR)\n"
            "# RESUME:       continue an EXISTING run (restores optimizer, scheduler, epoch)\n"
            "#   Auto-recovery: if 'last_harmonizer.pt' already exists locally (e.g. from\n"
            "#   a crashed session), RESUME takes priority over LOAD_WEIGHTS automatically.\n"
            "LOAD_WEIGHTS_CHECKPOINT = 'best_harmonizer_quality.pt'  # source run checkpoint\n"
            "RESUME_CHECKPOINT = ''          # set to 'best_harmonizer_quality.pt' to resume\n"
            "RESUME_RUN_NAME = RUN_NAME      # which Drive run to copy the resume checkpoint from\n"
            "ADDITIONAL_EPOCHS = 0           # used with --resume; 0 = use TRAIN_EPOCHS from config\n"
            ...
            "DRIVE_LOAD_WEIGHTS_DIR = Path(DRIVE_RUNS_DIR) / LOAD_WEIGHTS_RUN_NAME / 'checkpoints'\n"
            "DRIVE_RESUME_DIR = Path(DRIVE_RUNS_DIR) / RESUME_RUN_NAME / 'checkpoints'\n"
        ...
        code(
            ...
            "    # Use exactly RAMDISK_SIZE_GB, but never leave less than 64 GB free for training.\n"
            "    eff = min(RAMDISK_SIZE_GB, max(4, avail_gb - 64))\n"
            "    print(f'RAM available: {avail_gb} GB | ramdisk requested: {RAMDISK_SIZE_GB} GB | will mount: {eff} GB')\n"
            "    RAM_ROOT.mkdir(parents=True, exist_ok=True)\n"
            "    already_mounted = subprocess.run(['mountpoint', str(RAM_ROOT)], capture_output=True).returncode == 0\n"
            "    if already_mounted:\n"
            "        # Check actual mounted size; remount if it differs from requested.\n"
            "        df = subprocess.run(['df', '-BG', str(RAM_ROOT)], capture_output=True, text=True)\n"
            "        mounted_gb = 0\n"
            "        for line in df.stdout.splitlines()[1:]:\n"
            "            parts = line.split()\n"
            "            if parts:\n"
            "                try:\n"
            "                    mounted_gb = int(parts[1].rstrip('G'))\n"
            "                except ValueError:\n"
            "                    pass\n"
            "        if mounted_gb != eff:\n"
            "            print(f'Ramdisk already mounted at {mounted_gb} GB (need {eff} GB) — remounting...')\n"
            "            subprocess.run(['umount', str(RAM_ROOT)], check=False)\n"
            "            already_mounted = False\n"
            "        else:\n"
            "            print(f'Ramdisk already mounted at {mounted_gb} GB — reusing.')\n"
        ...
        code(
            ...
            "\n"
            "LOGDIR = PROJECT_ROOT / \"outputs\" / \"logs\"\n"
        ...
        code(
            ...
            "# ── Checkpoint strategy ──────────────────────────────────────────────────\n"
            "# Priority: auto-recovery from local last.pt > RESUME_CHECKPOINT > LOAD_WEIGHTS\n"
            "_local_last = LOCAL_CHECKPOINTS / 'last_harmonizer.pt'\n"
            "if _local_last.exists():\n"
            "    # Auto-recovery: session crashed mid-training, last checkpoint is local\n"
            "    print(f'Auto-recovery: found local {_local_last} — using --resume')\n"
            "    cmd += ['--resume', str(_local_last)]\n"
            "    if ADDITIONAL_EPOCHS > 0:\n"
            "        cmd += ['--additional-epochs', str(ADDITIONAL_EPOCHS)]\n"
            "elif RESUME_CHECKPOINT:\n"
            "    local_resume = LOCAL_CHECKPOINTS / RESUME_CHECKPOINT\n"
            "    drive_resume = DRIVE_RESUME_DIR / RESUME_CHECKPOINT\n"
            "    if not local_resume.exists():\n"
            "        if drive_resume.exists():\n"
            "            local_resume.parent.mkdir(parents=True, exist_ok=True)\n"
            "            shutil.copy2(drive_resume, local_resume)\n"
            "            print('copied resume checkpoint from Drive ->', local_resume)\n"
            "        else:\n"
            "            raise FileNotFoundError(f'Resume checkpoint not found: {drive_resume}')\n"
            "    cmd += ['--resume', str(local_resume)]\n"
            "    if ADDITIONAL_EPOCHS > 0:\n"
            "        cmd += ['--additional-epochs', str(ADDITIONAL_EPOCHS)]\n"
            "elif LOAD_WEIGHTS_CHECKPOINT:\n"
            "    local_load = LOCAL_CHECKPOINTS / LOAD_WEIGHTS_CHECKPOINT\n"
            "    drive_load = DRIVE_LOAD_WEIGHTS_DIR / LOAD_WEIGHTS_CHECKPOINT\n"
            "    if not local_load.exists():\n"
            "        if drive_load.exists():\n"
            "            local_load.parent.mkdir(parents=True, exist_ok=True)\n"
            "            shutil.copy2(drive_load, local_load)\n"
            "            print('copied load-weights checkpoint from Drive ->', local_load)\n"
            "        else:\n"
            "            raise FileNotFoundError(drive_load)\n"
            "    cmd += ['--load-weights', str(local_load)]\n"
