...
def build_notebook() -> dict:
    ...
    cells = [
        ...
        code(
            ...
            "print('Дальше: «loading PyTorch…», JSON (train_start / val_begin / train_step / epoch_end). -u + PYTHONUNBUFFERED; stdio inherited (no subprocess.PIPE) — in Colab, Interrupt/Stop works; PIPE + CUDA child used to hang the kernel.')\n"
            "def _stream_cmd(cmd, cwd, env):\n"
            "    # Do not use PIPE: parent blocked in read() while the child is in GPU; Colab then often cannot stop or restart the runtime.\n"
            "    r = subprocess.run(cmd, cwd=cwd, env=env)\n"
            "    if r.returncode != 0:\n"
            "        print(\n"
            "            f'\\n[exit {r.returncode}] Причина — в Traceback/ошибке ВЫШЕ в этой ячейке. CalledProcessError — только итог.',\n"
            "            flush=True,\n"
            "        )\n"
        ...
        code(
            ...
            "        r = subprocess.run(cmd, cwd=cwd, env=env)\n"
            "        if r.returncode != 0:\n"
            "            print(f'\\n[exit {r.returncode}] See Traceback above in this cell.', flush=True)\n"
            "            raise subprocess.CalledProcessError(r.returncode, cmd)\n"
            "env = os.environ.copy()\n"
            "env['PYTHONPATH'] = str(PROJECT_ROOT)\n"
            "env['PYTHONUNBUFFERED'] = '1'\n"
